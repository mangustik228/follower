...
def parsing(urls):
    ...
    with sync_playwright() as p:
        ...
        for url in tqdm(urls, desc='tiktok'):
            try:
                page.goto(url)
        ...
        browser.close()
    data = pd.DataFrame(
        columns=['ТикТок', 'tik_followers', 'tik_likes'], data=data)
    data['tik_followers'] = data['tik_followers'].apply(check_thousands)
    data['tik_likes'] = data['tik_likes'].apply(check_thousands)
    return data
